 ...
 *
 * Receives SNS notifications from SES bounce/complaint events
 * and sends alert messages to a Slack Incoming Webhook,
 * including details of which email addresses bounced or complained.
 *
 * Flow:
...
// ---------------------------------------------------------------------------
// Types (inline — the Lambda doesn't share lib/ with the Next.js app)
// ---------------------------------------------------------------------------
...
/**
* Extract the queue name from eventSourceARN
...
/**
* Resolve the Slack target for a given queue (falls back to global config)
...
/**
* Supports both SQS events (wrapped SNS) and direct SNS events
* (in case the Lambda is ever triggered directly from SNS)
...
function extractSnsMessage(record) {
 ...
 // Case: SNS → Lambda directly
 if (record.Sns?.Message) {
   return record.Sns.Message;
 }

 // Case: SQS → Lambda (SNS envelope)
 if (record.body) {
   try {
     const parsed = JSON.parse(record.body);
     // If it's an SNS envelope
     if (parsed?.Message) return parsed.Message;
     // If it's a direct payload (e.g. raw SES notification sent directly)
     return parsed;
   } catch {
     return undefined;
   }
...
/**
* Build a Slack Block Kit message for a bounce event
...
/**
* Build a Slack Block Kit message for a complaint event
* @param {SesNotification} notification
* @param {{ projectLabel: string, queueName?: string }} meta
*/
 ...
 for (const record of event.Records ?? []) {
   try {
     ...
     if (notificationType === "Bounce") {
       ...
       // Delivery notifications — not sent to Slack (volume is too high)
